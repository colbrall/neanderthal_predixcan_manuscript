...
# calculates and plots the difference of expression between all pairs of individuals given in args for each tissue
  ...
  for i in 1:(length(args)-1)
    ...
    for j in (i+1):length(args)
      ...
      for k in 2:length(names(ind1))
        const tmp = dropmissing(DataFrame(x1 = ind1[names(ind1)[k]],x2 = ind2[names(ind1)[k]]))::DataFrames.DataFrame
...
# writes file for each pair of inds containing the values and difference between them for each gene model
# Output: Gene Ind1 Ind2 Diff Tiss
function diffExpGenes(args::Array{String,1})
  for i in 1:(length(args)-1)
    ...
    for j in (i+1):length(args)
      ...
      for k in 2:length(names(ind1))
        const tmp = dropmissing(DataFrame(gene = ind1[names(ind1)[1]],x1 = ind1[names(ind1)[k]],x2 = ind2[names(ind1)[k]]))::DataFrames.DataFrame
